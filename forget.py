...
def main(cfg):
    ...
    if os.environ.get('LOCAL_RANK') is not None:
        ...
        device_map = {'': local_rank}  # 多卡训练的场景中，通过 local_rank 来指示每个进程对应的设备（GPU）
    #####################################################
    #init_process_group(backend="nccl", rank=local_rank, world_size=num_devices)
    ...
    # 设置为 "true" 时，意味着禁用 WANDB 的功能，
    # 防止它进行任何数据记录、上传或其他操作。
    # 这样做通常用于不希望将实验数据上传到 WANDB 服务的情况，或者在某些环境下避免网络请求等额外开销。
    model_cfg = get_model_identifiers_from_yaml(cfg.model_family)
    #model_id = model_cfg["hf_key"]
    model_id = model_cfg["model_path"]
    ...
    tokenizer = AutoTokenizer.from_pretrained("/disk/disk_20T/share/semeval25/OLMo_tokenizer")
    ...
    #     return {'Totalaaa': total_num, 'Trainableaaa': trainable_num}
    #
